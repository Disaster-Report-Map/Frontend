...
const DisasterMap = dynamic(
  () => import("../../../../components/map/DisasterMap"),
  {
    ssr: false,
    loading: () => (
      <div className="flex items-center justify-center h-[600px] w-full bg-slate-900 border border-slate-800 rounded-lg shadow-sm text-slate-400">
        Loading map...
      </div>
    ),
  },
...
export default function DashboardPage() {
  ...
  const [forcedCenter, setForcedCenter] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
    ...
    const baseMarkers = [
      {
        ...
        title: "Flood Level Rising - Sector 4",
        category: "flood",
        status: "active",
      },
      {
        lat: userPos.lat - 0.01,
        lng: userPos.lng + 0.008,
        title: "Warehouse Fire",
        category: "fire",
        status: "active",
      },
      {
        lat: userPos.lat + 0.015,
        lng: userPos.lng + 0.015,
      ...
      {
        lat: userPos.lat + 0.001,
        lng: userPos.lng + 0.002,
        title: "Secondary Flooding",
        category: "flood",
        status: "active",
      },
      {
        lat: userPos.lat - 0.002,
        lng: userPos.lng - 0.001,
        title: "Chemical Fire Exposure",
        category: "fire",
        status: "active",
      },
      {
        lat: userPos.lat + 0.003,
        lng: userPos.lng - 0.003,
        title: "Highway Collision",
        category: "accident",
        status: "active",
      },
      {
        lat: userPos.lat - 0.004,
        lng: userPos.lng + 0.004,
        title: "Heart Attack Report",
        category: "medical",
        status: "pending",
      },
      ...dynamicMarkers,
  ...
  const handleReportSubmit = (e: React.FormEvent) => {
    ...
    setDynamicMarkers((prev) => {
      ...
      try {
        ...
        console.warn("Local storage write fail", e);
      }
      return updated;
    });

    toast.success(
      `${reportForm.category.toUpperCase()} Report Submitted successfully`,
      {
    ...
    <div className="relative w-full h-full flex-1 overflow-hidden font-sans">
      ...
      <div className="absolute inset-0 z-0 bg-slate-800/50">
        <DisasterMap
          ...
          radarCenter={
            userPos
              ? { lat: userPos.lat + 0.005, lng: userPos.lng - 0.005 }
      ...
      <div className="absolute top-4 left-4 z-[400] bg-slate-900/90 backdrop-blur py-2 px-3 sm:py-3 sm:px-5 rounded-lg border border-slate-700/50 shadow-xl pointer-events-none max-w-[200px] sm:max-w-xs transition-opacity duration-300">
        <h1 className="text-sm sm:text-xl font-bold text-slate-100 leading-tight">
          Disaster Reports
        </h1>
        <p className="text-slate-400 text-[10px] sm:text-xs mt-0.5 leading-snug">
          Real-time overview of Disasters happening around you
        </p>
      </div>
      ...
      <MapSearch
        onLocationSelect={(lat, lng) =>
          setForcedCenter({ lat, lng, t: Date.now() } as any)
        }
      />
        ...
        <>
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl z-[1000] p-6 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-xs text-slate-500 mb-5">
              Location locked at: {draftLocation.lat.toFixed(4)},{" "}
              {draftLocation.lng.toFixed(4)}
            </p>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setReportForm({ ...reportForm, category: item.id })
                      }
